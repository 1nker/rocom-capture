...
**前端**:图层栏「野生宠物」的条件由 `WILD_LAYERS` 定义,选择记忆于 localStorage:
...
| 婉转声MAX / 粗嗓门MAX | `voice-highest` / `voice-lowest` | 关 |
...
| `voice-highest` | 协议原始 `voice == 100` |
| `voice-lowest` | 协议原始 `voice == -100` |

`OR` / `AND` 是两套**分别记忆、同时生效**的条件页签,按钮高亮只表示当前正在编辑哪套:

- `OR` 条件集:命中任一已勾选条件即显示;
- `AND` 条件集:体重组内取 OR、声音组内取 OR,已选择的不同属性组间取 AND;
- 两套结果最终取并集。体重/嗓音及四个 MAX 都可加入任一条件集,MAX 加入 `AND` 时归入对应组;
- 「异色/炫彩」「污染」不参与两套条件,使用跨页签共享的独立开关,命中即附加显示。

三套选择以 `.v4` 对象存储;旧 `.v3` 单条件集按原先保存的 OR/AND 模式迁入对应条件集,
异色/炫彩与污染迁入共享开关;更早 `.v2` 的 `voice`(原本就是精确 +100)迁移为 `voice-highest`。
`map.wildMatchMode.v1` 现在只记忆侧栏最后编辑的页签,不再关闭另一套条件。

标记是圆形头像(异色个体用异色头像)+ 类别描边:每个开关有独立颜色,复合命中时
`wildRing` 把当前开启且命中的条件色做等权 RGB 混合;MAX 会取代同方向普通条件色,
避免同一维度重复加权。标记不可点击,只有悬浮说明,故用 `cursor: help`,悬停放大 1.15×。
提示格式与事件页那行对齐:
...
`W` 是体重在本形态取值范围内的百分位(后端 `pet.SizePercentile` 算好放进 `weightPct`,
与宠物列表/事件页同一口径),固定显示两位小数;只有协议原始体重精确等于
`weight_low`/`weight_high` 时才显示 `0.00%`/`100.00%`。`V` 是嗓音原值。
数据经 SSE(`wildpets`)+ 加载时 `GET /api/wildpets` 回显;前端只绘制载荷
`sceneResId` 与当前地图一致的列表,避免位置与图层两条 SSE 在切场景时顺序不同而画错底图。
...
- **离开 AOI**(走远了,或被**别人**捉走了——这两者确实无从区分):不立刻抹掉,标 `stale`
  ...
  跨场景则按 `scene_res_id` 缓存在内存中(`wildByRes`),新场景只显示自己的目标,返回原场景时恢复;
  离开场景的时刻作为仍在 AOI 目标的最后所见时间,所有场景的灰点都在 4 小时后过期,服务重启后清空;
  灰点**照样计入侧栏图层行的数量**(计数与图上标记一一对应),悬浮该行看「视野内 N · 已离开视野 M」的拆分;
