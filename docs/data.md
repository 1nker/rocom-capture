...
> 判定(见 3.3);曾依赖策划字段的星点→区域几何归属已整体取消(见 3.4)。
...
服务器对紫星的 belong_camp 区域计数待新区 pcap 验证。
...
**星点→区域的几何归属已取消**(2026-07):曾按 AREA_CONF 里以区域名标注的多边形(126 行,
靠已被剥离的策划字段识别)给 POI 标 `z`,区域 `got>=tot` 时前端整片隐藏。剥离后分区多边形在
发布包里**没有第二来源**,靠旧版快照只会越来越过时(新增有星区域无从识别),故整个归属机制
连同快照一并移除——收集判定**纯逐点**(3.4 的实体/挂件判定),区域进度仅作数字展示。
> **当时找过的替代判据,全部不成立**(勿重蹈):AREA_CONF 存活字段无区分度(area_type 混同、
> 字段集一致);行 id 无编号段规律(同 id 段里还有 1541 个其他多边形);`AREA_FUNC_CONF.name`
> 虽是存活的区域名(进区域时的屏幕播报),但其 `area_id` 指向的是**播报触发体**而非计数分区——
> 拿它归属会有 265/477 点错位、46 点落多区(播报区互相重叠,如「风眠圣所」的播报区盖到风息山口);
> `NPC_REFRESH_CONTENT_CONF.belong_camp`(星星那几百行全为空,该字段只有野生宠物在用)、
> `CAMP_CONTENT_NPC_CONF`(空表)、「最近营地」归属(65 组只对上 11 组)。
...
不计区域(计入 94)。故**不要拿配置点数当分母**算进度,进度一律用服务器给的 `explore_num/total_num`,
且 `got>=tot` 才算收满。被移除的那颗光点实体永不下发,逐点判定会把它标成「已收集」——对没收过的人
它本也不可再得,可接受。
