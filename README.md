...
- **页面三 · 实时地图**：登录账号自己在大地图上的实时位置与朝向，进入洞穴/家园楼层时叠加分层地图，支持缩放平移；
  ...
  另有「野生宠物」图层：把周围刷出的稀有个体(异色/炫彩、污染、满声音)标在地图上——
  这些属性捕捉前后一致，丢球之前就能筛(见 docs/data.md 3.5)。
  在**家园**里另有「精灵小窝」图层：10 个小窝的位置与入住的宠物(空窝也画出来)，
  悬浮看住户简要信息(尺寸百分位/声音/性格/喂食轮数/配对的另一半)，点头像看宠物详情；
  窝上还没收的蛋会挂个蛋图标。
- **页面四 · 精灵蛋**：背包里的每颗蛋——蛋图、孵出物种、身高/体重(含在该物种蛋区间内的百分位)、
  获得时间、来源、孵化进度；普通蛋还按百分位算出孵化后的成体尺寸(实测原样保留，见 docs/data.md 3.6)。
